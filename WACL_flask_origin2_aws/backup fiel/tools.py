...
import numpy as np
import json
pic_path = 'C:\\Users\\jacky\\PycharmProjects\\WACL_flask_origin\\static\\'
#pic_path = "C:\\Users\\jacky\\Desktop\\person_pic\\"
def jsondump_processing(payload):
    try:
        ...
        pic_restored = np.array(json_load['pic'], dtype=np.float_)
        file_path = path_maker(p_id_restored, cam_id_restored, s_time_restored)
        #print(file_path)
        cv.imwrite(file_path, pic_restored)
        data_ = [p_id_restored,s_time_restored,e_time_restored,file_path]
        return data_
...
def path_maker(p_id,cam_id,s_time):
    path = pic_path + "Pid_"+ str(p_id) + "_cid_"+str(cam_id) + "_" + s_time + ".jpg"
    return path
